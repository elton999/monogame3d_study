...
VertexShaderOutput MainVS(in VertexShaderInput input)
{
    ...
    matrix skinMatrix = mul(input.Weights.x, Bones[int(input.Joints.x)]);
    skinMatrix += mul(input.Weights.y, Bones[int(input.Joints.y)]);
    skinMatrix += mul(input.Weights.z, Bones[int(input.Joints.z)]);
    skinMatrix += mul(input.Weights.w, Bones[int(input.Joints.w)]);
